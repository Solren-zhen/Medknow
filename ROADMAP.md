...
1. ~~**视图（AP/PA）混杂分析**~~（2026-09-13 完成，结论见
   `results/tables/view_confounder_nih.md`）：判别力崩塌**不能**由视图混杂解释
   （NIH-PA 0.619 vs NIH-AP 0.623，ΔAUC −0.004，p=0.98；同视图下内部 0.992 →
   NIH-PA 0.619 依然崩塌）；新发现：转诊信号呈**视图结构化失效**
   （误差预测 AUC：PA 层 0.746 可用、AP 层 0.236 反向，混合后 ≈0.49）——
   原假设被否定，但产出更强的论文论点，待写入手稿
