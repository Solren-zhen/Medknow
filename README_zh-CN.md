...
任何一个上线运行的医学 AI 系统，都在悄悄替人做决定：**哪些影像有人工复核、
哪些直接放行**。基于置信度的分诊假设"模型知道自己什么时候会错"——本项目证明
这个假设恰恰在真正部署的场景（换一家医院、换一台设备）下失效，并提供了一套
可复现的评估协议（患者级切分、外部队列、校准、转诊曲线、误差模式），
用来回答医院设备委员会真正会问的问题：**这个模型在我们这里可信吗？
它应该把哪些病例转给人？** 适合的读者：想学会审视医学 AI 的临床医生与学生，
以及需要外部验证参考流程的工程师。
